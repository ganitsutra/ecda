...
let ploop p lst oc =
	let combiner acc x =
		let output = p (Extracted_tree.bt acc) (Extracted_tree.at acc) x (Extracted_tree.b_id acc) (Extracted_tree.a_id acc) in 
		(printmfile oc (Extracted_tree.mt output)); output
	in
	let init = ((((Extracted_tree.TB.empty, Extracted_tree.TA.empty),Extracted_tree.T_id.empty),Extracted_tree.T_id.empty),[])
	in
...
ignore (ploop (Extracted_tree.process_instruction) orderbook oc) ; close_out oc;;
