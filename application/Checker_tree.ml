...
|"Buy" -> Extracted_tree.Buy
|"Sell" -> Extracted_tree.Sell
|"Del" -> Extracted_tree.Del
...
let create_instruction l=
        match l with
                |[c;i;t;q;p] -> {Extracted_tree.cmd = convert_to_command c; Extracted_tree.ord ={ Extracted_tree.id =  int_of_string i; Extracted_tree.otime = int_of_string t; 
                Extracted_tree.oprice = int_of_string p;Extracted_tree.oquantity = int_of_string q }}
...
let rec printmfile oc (m:Extracted_tree.transaction list) = match m with 
  |[] -> ()
  |[f] -> Printf.fprintf oc "%d,%d,%d\n" f.idb f.ida f.tquantity
  |f::m' -> Printf.fprintf oc "%d,%d,%d;" f.idb f.ida f.tquantity; printmfile oc m'

let writematching file (m:Extracted_tree.transaction list) =
...
let ploop p lst oc =
	let combiner acc x =
		let output = p (Extracted_tree.bt acc) (Extracted_tree.at acc) x (Extracted_tree.bt_id acc) (Extracted_tree.at_id acc) in 
		(printmfile oc (Extracted_tree.mt output)); output
	in
	let init = ((((Extracted_tree.TB.Leaf, Extracted_tree.TA.Leaf),Extracted_tree.T_id.Leaf),Extracted_tree.T_id.Leaf),[])
	in
	List.fold_left combiner init lst ;;
(**  
let rec ploop2 p i oc =
  match i with
  	|[] -> let output = ((((Extracted_tree.TB.empty, Extracted_tree.TA.empty),Extracted_tree.T_id.empty),Extracted_tree.T_id.empty),[]) in (printmfile oc (Extracted_tree.mt output));
  	       output
  	|head::tail -> let output = (let it = ploop2 p tail oc in
          p (Extracted_tree.bt it) (Extracted_tree.at it) head (Extracted_tree.b_id it) (Extracted_tree.a_id it)) in (printmfile oc (Extracted_tree.mt output)); output
...
let lexicographic_compare (x:Extracted_tree.transaction) (y:Extracted_tree.transaction) =
...
let create_transaction l =
        match l with
                |[i;j;q] -> {Extracted_tree.idb = int_of_string i; Extracted_tree.ida = int_of_string j; Extracted_tree.tquantity = int_of_string q }
                |_ ->raise (Invalid_argument "instruction information is incomplete");;
...
(**3. This function takes a list of strings and converts them into list of transactions*)
let rec printm (m:Extracted_tree.transaction list) str = match m with
	|[] -> ()
...
|(line1::l1', line2::l2') -> let m1 = string_to_transaction_list line1 in
			   ...
			   let b = compare_matchings (Extracted_tree.cform m1) (Extracted_tree.cform m2) in
...
(**f := (string_to_tuple) and l := mylist  map:=
 str_list_to_instructions **)

let m = List.length orderbook;;
Printf.printf "\n%s contains %d instructions.\n" iorderfile m;;
let ofile = "verified_tree_" ^ Sys.argv.(1) ;;


let () = let oc = open_out ofile in
ignore (ploop (Extracted_tree.eProcess_instruction) orderbook oc) ; close_out oc;;
